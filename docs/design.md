...
2. **Measurement layer** — ruler plus cone/circle/rect templates,
   bound to grid units.
3. **Walls + dynamic lighting** — walls as line segments; visibility
   polygons; darkness layer punched by light masks. The same wall
   data later drives token vision.
4. **Dynamic sound** — ambient emitters with position/radius/falloff;
...
Stretch: an SRD bestiary panel — search "goblin", drag it onto the
board.

**Acceptance scene:** load a tavern map, place walls, drop three
tokens, measure a move, splash a fireball template, light a torch on
a token, put crackling-fire ambience on the hearth.
